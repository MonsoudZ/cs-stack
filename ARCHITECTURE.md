...
The main stack (`src/pages/index.astro`) and all 17 deep dives (`/silicon`,
...
| `stacks.js` | the 17 deep dives: `slug, name, kind, layer, accent, blurb` | `StackNav` footer, `GoDeeper`, `SeeAlso`, per-stack OG cards, `curriculum` |
| `curriculum.js` | the guided-path order (overview + 17), derived from `stacks.js`; `key` = localStorage progress id | `/learn`, `PrevNext` |
